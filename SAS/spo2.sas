...
data treatment_2;
    ...
    if VAR2='A' then GROUP='Ž©”­ŒÄ‹zŒQ(SPŒQ)';
...
%macro COUNT (name, var, rdata, a2z, title);
    ...
    data &name._sp &name._mr;
        ...
        by SUBJID;
        if a;
        if GROUP='Ž©”­ŒÄ‹zŒQ(SPŒQ)' then output &name._sp;
...
%COUNT (x_SpO2_n, answer, ptdata, %NRSTR('‚ ‚è', '‚È‚µ'), %NRSTR(SpO2<95%‚Æ‚È‚Á‚½‰ñ”(ƒmƒCƒY‚ðœ‚­)));
...
data spo2;
    ...
    total=col1+col2;
    drop _LABEL_;
    label col1='’áŽ_‘fƒCƒxƒ“ƒg‚ ‚è' col2='‚È‚µ' total='‡Œv'  _NAME_='type';
run;
