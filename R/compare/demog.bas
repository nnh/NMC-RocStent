Option Explicit
 ...
 Dim output_row As Integer
    ...
    Next i
    For i = 1 To 1000
        If output_ws.Cells(i, 1).Value = "気道の狭窄部位(主気管を含むか否か)" Then
            output_ws.Cells(i, 1).Value = "気道の狭窄部位(主気管を含む)"
        End If
        If output_ws.Cells(i, 1).Value = "予定、緊急" Then
            output_ws.Cells(i, 1).Value = "予定手術または緊急手術"
        End If
    Next i
 End Sub
