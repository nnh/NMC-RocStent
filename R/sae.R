...
# ' output:
# '   html_document:
SaeReport_Group <- function(column_name, df_group_a, df_group_b, df_a_count, df_b_count){
  ...
  # set percentage
  sae_table[ ,3] <- round(sae_table[ ,2] / df_a_count * 100, digits=1)
  sae_table[ ,5] <- round(sae_table[ ,4] / df_b_count * 100, digits=1)
...
#' # n=`r all_treatment`
#' ## 重篤な有害事象発生割合
output_sae <- SaeReport_Group("sae1_trm", sp_sae_report, mr_sae_report, nrow(sp_ptdata), nrow(mr_ptdata))
...
for (i in kCTCAEGrade) {
  ...
  if (nrow(temp_sp) > 0 | nrow(temp_mr) > 0) {
    temp_output <- SaeReport_Group(temp_colname, temp_sp, temp_mr, nrow(sp_ptdata), nrow(mr_ptdata))
  } else {
